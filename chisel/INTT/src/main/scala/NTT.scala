...
class INTTWrap(conf:Config) extends Module{
    val io = IO(new Bundle{
        val in = Input(UInt((conf.radix*conf.qbit).W))
        val out = Output(UInt((conf.radix*64).W))
        val validout = Output(Bool())
        val enable = Input(Bool())
    })
    ...
    io.out := Cat(buf.reverse)
    io.validout := intt.io.validout

    intt.io.enable := io.enable
    for(j <- 0 until conf.chunk){
        for(k <- 0 until conf.radix){
            val index = k*conf.chunk+j
            intt.io.in(j)(k) := io.in((index+1)*conf.qbit-1,index*conf.qbit)
        }
    }
    io.out := Cat(intt.io.out(0).reverse)
}

object INTTWrapTop extends App {
  (new chisel3.stage.ChiselStage).emitVerilog(new INTTWrap(Config()))
}