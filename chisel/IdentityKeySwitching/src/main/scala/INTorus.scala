...
class INTorusLSH(implicit val conf:Config) extends Module{
    ...
    when(io.l<32.U){
        ...
        val res = templ +& ((tempu<<32)(63,0) - tempu)
        ...
        val res = ((templ +& tempul) << 32.U)(63,0) - tempuu - tempul
        val resmod = Mux(tempul===0.U,Mux(res > (templ<<32), res - ((1L<<32)-1).U, res), Mux(res < (templ<<32), res + ((1L<<32)-1).U, res))
        io.Y := Mux(resmod>=conf.P.asUInt,resmod+((1L<<32)-1).U,resmod)
    }.elsewhen(io.l < 96.U){
