...
class ExternalProductPreMiddle(implicit val conf:Config) extends Module{
    ...
    val inttenablereg = RegInit(false.B)

    when(io.axi4sin(0).TVALID && RegNext(~io.axi4sin(0).TVALID)){
        inttenablereg := true.B
    }
    ...
    val inttselreg = RegInit(0.U(1.W))
    val digitreg = RegInit(0.U(log2Ceil(conf.l).W))
    when(intt.io.fin){
        when(digitreg === (conf.l - 1).U){
            digitreg := 0.U
...
class ExternalProductMiddle(implicit val conf:Config) extends Module{
    ...
    for(i <- 0 until conf.nttnumbus){
        io.axi4sout(i).TVALID := mulandacc.io.valid
...
class ExternalProductLater(implicit val conf:Config) extends Module{
    ...
    val nttenablereg = RegInit(false.B)
    ntt.io.enable := nttenablereg
    when(io.axi4sin(0).TVALID){
        nttenablereg := true.B
    }

    val tdatavec = Wire(Vec(conf.nttnumbus,UInt(conf.buswidth.W)))
    for(i <- 0 until conf.nttnumbus){
        io.axi4sin(i).TREADY:= ntt.io.ready
        tdatavec(i) := io.axi4sin(i).TDATA
    }
    for(i <- 0 until conf.radix){
...
object ExternalProductWrapState extends ChiselEnum {
  val WAIT,INIT, BUBBLE, RUN, OUT = Value
}

class ExternalProductWrap(implicit val conf:Config) extends Module{
    val io = IO(new Bundle{
        ...
        val inttvalidout = Output(Bool())
        val inttout = Output(UInt((conf.fiber*64).W))
        val accout = Output(UInt((2*conf.block*64).W))
	})

    io.fin := false.B

    val inlbuf = Module(new Lbuffer(conf.Qbit,0,conf))
    for(i <- 0 until conf.radix){
        inlbuf.io.in(0)(i) := io.in((i+1)*conf.Qbit-1,i*conf.Qbit)
    }
    inlbuf.io.validin := io.validin

    val extp = Module(new ExternalProduct)
    io.inttvalidout := extp.io.inttvalidout
    io.inttout := Cat(extp.io.inttout.flatten.reverse)
    io.accout := extp.io.accout
    io.out := Cat(extp.io.out(0).reverse)

    extp.io.in := inlbuf.io.out
    extp.io.validin := inlbuf.io.validout
    io.validout := extp.io.validout
    extp.io.trgswin := io.trgswin
    extp.io.trgswinvalid := io.trgswinvalid
    io.trgswinready := extp.io.trgswinready

    val finreg = RegInit(0.U(2.W))
    when(RegNext(extp.io.fin)){
        finreg := finreg +1.U
    }
    when(finreg===2.U){
        io.fin := true.B
...
object ExternalProductWrapTop extends App {
  (new chisel3.stage.ChiselStage).emitVerilog(new ExternalProductWrap()(Config()))
}