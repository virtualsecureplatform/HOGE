...
#include <VHomNANDWrap.h>
#include <tfhe++.hpp>

void clock(VHomNANDWrap *dut, VerilatedFstC* tfp){
...
int main(int argc, char** argv) {
  ...
  VHomNANDWrap *dut = new VHomNANDWrap();
  ...
  std::cout<<"Initialize"<<std::endl;
  //Initialize Input Buffer
  for(int i = 1; i < alignedlenlvl1/buswords; i++){
    for(int j = 0; j < buswords; j++){
      dut->io_axi4ina_TDATA[j] = tlweaaligned[buswords*i+j];
      dut->io_axi4inb_TDATA[j] = tlwebaligned[buswords*i+j];
    }
    dut->eval();
    for(int j = 0; j < buswords; j++){
...
for (int i = 0; i < TFHEpp::lvl0param::n; i++) {
  ...
  for(int cycle = 0; cycle<numcycle;cycle++){
    ...
    if(dut->io_debugout[m]!=brres[0][cycle*radix+m]){
      std::cout<<i<<":0:"<<cycle<<":"<<m<<std::endl;
      std::cout<<"ERROR:"<<brres[0][cycle*radix+m]<<":"<<dut->io_debugout[m]<<std::endl;
      dut->final();
      tfp->close(); 
      exit(1);
    }
  ...
  for(int cycle = 0; cycle<numcycle;cycle++){
    for(int m = 0; m<radix;m++)
    if(dut->io_debugout[m]!=brres[1][cycle*radix+m]){
      std::cout<<i<<":1:"<<cycle<<":"<<m<<std::endl;
      std::cout<<brres[1][cycle*radix+m]<<":"<<dut->io_debugout[m]<<std::endl;
      dut->final();
      tfp->close(); 
      exit(1);
    }
