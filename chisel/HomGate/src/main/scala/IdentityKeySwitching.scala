...
class AddressDecomposition(implicit val conf:Config) extends Module{
	...
	val outqueue = Module(new Queue(UInt(conf.basebit.W),2*conf.basebit, useSyncReadMem = true))
...
class AXISIKS(implicit val conf:Config) extends Module{
	...
	IKS.io.enable := RegNext(tlwe2index.io.validout)
	io.debug_iksenable := RegNext(tlwe2index.io.validout)
