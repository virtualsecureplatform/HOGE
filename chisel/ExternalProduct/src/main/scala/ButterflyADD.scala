...
class INTTradixButterflyUnit(radixbit : Int, embedtwist : Boolean) extends Module{
    val size = 1<<radixbit
    val lshdelay = 3
    ...
    if(radixbit > 1){
        val upper = Module(new INTTradixButterflyUnit(radixbit - 1,false))
        upper.io.in.zip(bus.slice(0,size/2)).foreach { case (a, b) => a:= b }
        io.out.slice(0,size/2).zip(upper.io.out).foreach { case (a, b) => a:= b }
        val lower = Module(new INTTradixButterflyUnit(radixbit - 1,false))
...
class DITradixButterflyUnit(forward: Bool, radixbit : Int, embedtwist : Boolean) extends Module{
    val size = 1<<radixbit
    val lshdelay = 3
    ...
    if(radixbit > 1){
        val upper = Module(new DITradixButterflyUnit(forward,radixbit - 1,false))
        upper.io.in.zip(bus.slice(0,size/2)).foreach { case (a, b) => a:= b }
        io.out.slice(0,size/2).zip(upper.io.out).foreach { case (a, b) => a:= b }
        val lower = Module(new DITradixButterflyUnit(forwared,radixbit - 1,false))
        lower.io.in.zip(bus.slice(size/2,size)).foreach { case (a, b) => a:= b }
        io.out.slice(size/2,size).zip(lower.io.out).foreach { case (a, b) => a:= b }
    }else{
        io.out := bus
