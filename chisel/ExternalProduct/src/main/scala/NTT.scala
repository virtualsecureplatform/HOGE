...
class INTT(implicit val conf:Config) extends Module{
    ...
    val formerinttbuts = for(i <- 0 until conf.chunk) yield{
        val inttbut = Module(new INTTradixButterflyUnit(conf.radixbit,true))
        inttbut
    }
    val laterinttbuts = for(i <- 0 until conf.chunk) yield{
        val inttbut = Module(new INTTradixButterflyUnit(conf.radixbit,false))
        inttbut
    }
...
// Assuming Queue(no bubble)
class BothNTT(val forward: Bool, implicit val conf:Config) extends Module{
    val io = IO(new NTTport)
