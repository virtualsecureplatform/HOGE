...
int main(int argc, char* argv[]) {
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, kernel_bootstrapping = cl::Kernel(program, "BootstrappingTop", &err));
            valid_device = true;
	...
	alignas(4096) TFHEpp::TLWE<TFHEpp::lvl0param> tlwelvl0,iksres={};
	TFHEpp::IdentityKeySwitch<TFHEpp::lvl10param>(tlwelvl0,tlwe,*iksk);
	...
	std::cout<<"tlwe:"<<std::hex<<tlwe[0]<<std::endl;
	std::cout<<"tlwelvl0:"<<std::hex<<tlwelvl0[0]<<std::endl;
	...
	double  kernel_time_in_sec;
	{
		...
		// cl_mem_ext_ptr_t
		// and provide the PCs
		cl_mem_ext_ptr_t inBuf, resBuf, iksresBuf;
		...
		inBuf.obj = tlwe.data();
		inBuf.param = 0;
		inBuf.flags = PC_NAME(1);

		iksresBuf.obj = iksres.data();
		iksresBuf.param = 0;
		iksresBuf.flags = PC_NAME(2);
		...
		for(int i = 0; i < bknumbus; i++){
			...
			bknttBufs[i].param = 0;
			bknttBufs[i].flags = pc[32-bknumbus+i];
		}
		...
		OCL_CHECK(err, cl::Buffer buffer_iksres(context, CL_MEM_WRITE_ONLY | CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR,
												sizeof(iksres), &iksresBuf, &err));
		...
		OCL_CHECK(err, err = (kernel_bootstrapping).setArg(1, buffer_in));
		OCL_CHECK(err, err = (kernel_bootstrapping).setArg(2, buffer_iksres));
		...
		OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_res,buffer_iksres}, CL_MIGRATE_MEM_OBJECT_HOST));
		q.finish();
	...
	for(int i = 0; i <= TFHEpp::lvl0param::n; i++){
		if(iksres[i] != tlwelvl0[i]){
			std::cout<<"ERROR: "<<i<<" : "<<iksres[i]<<" : "<<tlwelvl0[i]<<std::endl;
			exit(1);
		}
	}
	for(int k = 0; k < 2; k++)
