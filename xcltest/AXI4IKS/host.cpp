...
int main(int argc, char* argv[]) {
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, kernel_iks = cl::Kernel(program, "AXI4IKSTop", &err));
            valid_device = true;
	...
	constexpr uint numbus = 10;
	constexpr uint wordsinbus = 512/std::numeric_limits<typename TFHEpp::lvl0param::T>::digits;
 	constexpr uint alignedlenlvl0 = (((std::numeric_limits<TFHEpp::lvl0param::T>::digits*(TFHEpp::lvl0param::n+1)>>buswidthlb)+1)<<buswidthlb)/std::numeric_limits<TFHEpp::lvl0param::T>::digits;
  	alignas(4096) std::array<std::array<std::array<std::array<std::array<std::array<typename TFHEpp::lvl0param::T, wordsinbus>, 40/numbus>, (1 << TFHEpp::lvl10param::basebit) - 1>, TFHEpp::lvl10param::t>,TFHEpp::lvl1param::n>, numbus> ikskaligned = {};
  	for(int i = 0; i<TFHEpp::lvl1param::n; i++) for(int j = 0; j < TFHEpp::lvl10param::t; j++) for(int k = 0; k< (1 << TFHEpp::lvl10param::basebit) - 1; k++) for(int l = 0; l < wordsinbus; l++) for(int m = 0; m < numbus; m++) for(int n = 0; n < 40/numbus; n++) ikskaligned[m][i][j][k][n][l] = (*iksk)[i][j][k][n*numbus*wordsinbus+m*wordsinbus+l];
	...
	double  kernel_time_in_sec;
	{
		...
		std::array<cl_mem_ext_ptr_t, numbus> ikskBufs;
		...
		std::array<std::shared_ptr<cl::Buffer>, numbus> buffer_iksks;
		for(int i = 0; i < numbus; i++){
		...
		for(int i = 0; i < numbus; i++) OCL_CHECK(err, err = (kernel_iks).setArg(i+2, *(buffer_iksks[i])));

		// Copy input data to Device Global Memory
        std::cout<<"Copy"<<std::endl;
		std::vector<cl::Memory> inputs(numbus);
		for(int i = 0; i < numbus; i++) inputs[i] = *(buffer_iksks[i]);
