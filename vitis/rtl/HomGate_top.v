...
reg [31:0] dbg_dm_beats;       // DataMover output beats (TVALID & TREADY)
reg [31:0] dbg_dm_stallcnt;    // cycles with TVALID=0 after transfer started
reg        dbg_dm_sts_seen;    // DataMover status received
reg [7:0]  dbg_dm_sts_data;    // Status data
reg        dbg_dm_err_seen;    // mm2s_err asserted
reg [31:0] dbg_ar_cnt;         // AXI AR transactions issued
reg [31:0] dbg_r_cnt;          // AXI R beats received
always @(posedge ap_clk) begin
    if (!ap_rst_n) begin
        dbg_dm_beats <= 0;
        dbg_dm_stallcnt <= 0;
        ...
        if (m13_axi_arvalid & m13_axi_arready)
            dbg_ar_cnt <= dbg_ar_cnt + 1;
        if (m13_axi_rvalid & m13_axi_rready)
            dbg_r_cnt <= dbg_r_cnt + 1;
        if (axi4bkin_0_TVALID & axi4bkin_0_TREADY)
            dbg_dm_beats <= dbg_dm_beats + 1;
        if (datamover13_mm2s_sts_tvalid) begin
            dbg_dm_sts_seen <= 1;
            dbg_dm_sts_data <= datamover13_mm2s_sts_tdata;
            $fwrite(32'h80000002, "BK_DM_STS: beats=%0d sts=0x%02x err=%0d ar=%0d r=%0d\n",
                    dbg_dm_beats, datamover13_mm2s_sts_tdata, datamover13_mm2s_err,
                    dbg_ar_cnt, dbg_r_cnt);
        end
        if (datamover13_mm2s_err && !dbg_dm_err_seen) begin
            dbg_dm_err_seen <= 1;
            $fwrite(32'h80000002, "BK_DM_ERR: beats=%0d err=1 valid=%0d ready=%0d\n",
                    dbg_dm_beats, axi4bkin_0_TVALID, axi4bkin_0_TREADY);
        end
        // Detect when DataMover stops outputting (TVALID=0 while TREADY=1)
        if (dbg_dm_beats > 0 && !axi4bkin_0_TVALID && axi4bkin_0_TREADY) begin
            dbg_dm_stallcnt <= dbg_dm_stallcnt + 1;
        end else begin
            dbg_dm_stallcnt <= 0;
        end
        if (dbg_dm_stallcnt == 5000) begin
            dbg_dm_stallcnt <= 0;
            $fwrite(32'h80000002, "BK_DM_IDLE: beats=%0d ar=%0d r=%0d arv=%0d arr=%0d rv=%0d rr=%0d sts=%0d err=%0d\n",
                    dbg_dm_beats, dbg_ar_cnt, dbg_r_cnt,
                    m13_axi_arvalid, m13_axi_arready,
                    m13_axi_rvalid, m13_axi_rready,
                    dbg_dm_sts_seen, datamover13_mm2s_err);
        end
    end
end

// Debug: S2MM output path monitoring
reg [31:0] dbg_s2mm_beats;     // S2MM stream beats (TVALID & TREADY)
reg [31:0] dbg_s2mm_stallcnt;  // cycles with TVALID=1 but TREADY=0
reg [31:0] dbg_axis01_beats;   // axis01 input beats
reg        dbg_s2mm_started;   // first beat seen
reg        dbg_s2mm_cmd_accepted; // S2MM command accepted
reg        dbg_s2mm_done_printed; // one-shot for S2MM_DONE
always @(posedge ap_clk) begin
    if (!ap_rst_n) begin
        dbg_s2mm_beats <= 0;
        dbg_s2mm_stallcnt <= 0;
        dbg_axis01_beats <= 0;
        dbg_s2mm_started <= 0;
        dbg_s2mm_cmd_accepted <= 0;
        dbg_s2mm_done_printed <= 0;
    end else begin
        // Track S2MM command acceptance
        if (axi4outcmd_TVALID & axi4outcmd_TREADY & !dbg_s2mm_cmd_accepted) begin
            dbg_s2mm_cmd_accepted <= 1;
            $fwrite(32'h80000002, "S2MM_CMD: addr=0x%08x%08x BTT=0x%06x EOF=%0d TYPE=%0d\n",
                    axi4outcmd_TDATA[95:64], axi4outcmd_TDATA[63:32],
                    axi4outcmd_TDATA[22:0], axi4outcmd_TDATA[30], axi4outcmd_TDATA[23]);
        end
        // Track axis01 input from BRBack
        if (axis01_tvalid & axis01_tready)
            dbg_axis01_beats <= dbg_axis01_beats + 1;
        // Track S2MM stream data - print first 4 beats
        if (axi4sout_TVALID & axi4sout_TREADY) begin
            dbg_s2mm_beats <= dbg_s2mm_beats + 1;
            if (dbg_s2mm_beats < 4) begin
                $fwrite(32'h80000002, "S2MM_BEAT[%0d]: data=0x%08x\n", dbg_s2mm_beats, axi4sout_TDATA);
            end
            if (!dbg_s2mm_started) begin
                dbg_s2mm_started <= 1;
            end
        end
        // Detect stall: TVALID=1 but TREADY=0
        if (axi4sout_TVALID & !axi4sout_TREADY) begin
            dbg_s2mm_stallcnt <= dbg_s2mm_stallcnt + 1;
            if (dbg_s2mm_stallcnt == 5000) begin
                dbg_s2mm_stallcnt <= 0;
                $fwrite(32'h80000002, "S2MM_BACKPRESSURE: beats=%0d ax01v=%0d ax01r=%0d outV=%0d outR=%0d\n",
                        dbg_s2mm_beats, axis01_tvalid, axis01_tready,
                        axi4sout_TVALID, axi4sout_TREADY);
            end
        end else begin
            dbg_s2mm_stallcnt <= 0;
        end
        // Periodic status when waiting for data (no axis01 activity)
        if (dbg_s2mm_cmd_accepted & !dbg_s2mm_started & !axis01_tvalid) begin
            dbg_s2mm_stallcnt <= dbg_s2mm_stallcnt + 1;
            if (dbg_s2mm_stallcnt == 50000) begin
                dbg_s2mm_stallcnt <= 0;
                $fwrite(32'h80000002, "S2MM_WAITING: ax01v=%0d ax01r=%0d outV=%0d outR=%0d beats=%0d\n",
                        axis01_tvalid, axis01_tready,
                        axi4sout_TVALID, axi4sout_TREADY,
                        dbg_s2mm_beats);
            end
        end
        // Print when S2MM transfer completes (one-shot)
        if (dbg_s2mm_started & !axi4sout_TVALID & dbg_s2mm_beats > 0 & !dbg_s2mm_done_printed) begin
            if (dbg_s2mm_beats == dbg_axis01_beats) begin
                dbg_s2mm_done_printed <= 1;
                $fwrite(32'h80000002, "S2MM_DONE: beats=%0d ax01beats=%0d\n",
                        dbg_s2mm_beats, dbg_axis01_beats);
            end
        end
    end
end

// Debug: S2MM AXI4 write monitoring
reg [31:0] dbg_axi_wd_cnt;
always @(posedge ap_clk) begin
    if (!ap_rst_n) begin
        dbg_axi_wd_cnt <= 0;
    end else begin
        // Print AXI write address channel
        if (m00_axi_awvalid & m00_axi_awready) begin
            $fwrite(32'h80000002, "S2MM_AW: addr=0x%016x len=%0d\n",
                    m00_axi_awaddr, m00_axi_awlen);
        end
        // Print first 2 AXI write data beats (each 512-bit = 16 x 32-bit words)
        if (m00_axi_wvalid & m00_axi_wready) begin
            if (dbg_axi_wd_cnt < 2) begin
                $fwrite(32'h80000002, "S2MM_WD[%0d]: w0=0x%08x w1=0x%08x w2=0x%08x w3=0x%08x strb=0x%016x\n",
                        dbg_axi_wd_cnt,
                        m00_axi_wdata[31:0], m00_axi_wdata[63:32],
                        m00_axi_wdata[95:64], m00_axi_wdata[127:96],
                        m00_axi_wstrb);
            end
            dbg_axi_wd_cnt <= dbg_axi_wd_cnt + 1;
        end
    end
end
