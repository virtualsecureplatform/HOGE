...
template_result = Environment(loader=FileSystemLoader('.')).get_template("homgate.template").render(data) #Load template
with open("HomGate.v","w") as f:
    f.write(template_result) #generate verilog

data ={"buss":[[str(x+6).zfill(2),str(x)] for x in range(8)],"passbuss":[[str(x),str(x).zfill(2),str(x+4).zfill(2)] for x in range(4)]}
